...
def _enumerate_folders(
    ...
    *,
    ...
    Returns a list of folder entry dicts.
    """
    ...
    # Count items (read-only — DefaultItemType check is for mail folders)
    item_count = 0
    if not excluded:
        try:
...
def scan_primary_store(
    ...
    *,
    ...
    Returns the source catalog dict.
    """
    ...
    store = inbox.Store  # resolve the Store for the Inbox
    store_name = str(store.DisplayName) if store else "Unknown"
    store_id = str(store.StoreID) if store else ""

    # Step 2: get root folder and enumerate
    root_folder = store.GetRootFolder()
    now_iso = datetime.now(timezone.utc).isoformat()

    folder_entries = _enumerate_folders(
        root_folder,
        include_deleted=include_deleted,
        include_junk=include_junk,
        include_drafts=include_drafts,
    ...
    # Step 3: list excluded stores (metadata only, no folder scan)
