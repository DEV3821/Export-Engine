...
class TestSchemaSafetyFlags:
    ...
    SCHEMA_MAKERS = [
        ("source_catalog", lambda: new_source_catalog_entry(folder_path="Inbox", folder_type="inbox")),
    ...
    def test_all_schemas_have_safety_block(self) -> None:
        for name, maker in self.SCHEMA_MAKERS:
            schema = maker()
            assert "_safety" in schema, f"{name} missing _safety block"
            safety = schema["_safety"]
            assert isinstance(safety, dict), f"{name} _safety is not dict"
...
class TestSchemaStructure:
    """Individual schema structural checks."""
